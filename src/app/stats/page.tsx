...
function ChartCard({ title, note, children, className = '' }: { title: string; note: string; children: React.ReactNode; className?: string }) {
  return <Card className={`flex flex-col overflow-hidden border-border bg-card ${className}`}><div className="border-b border-border px-5 py-4"><h2 className="font-semibold text-foreground">{title}</h2><p className="mt-0.5 text-xs text-muted-foreground">{note}</p></div><CardContent className="min-h-0 flex-1 p-4 sm:p-5">{children}</CardContent></Card>;
}
