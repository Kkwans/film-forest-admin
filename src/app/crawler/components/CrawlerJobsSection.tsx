  ...
  return (
    <section className="space-y-4">
      ...
      {loading && jobs.length === 0 ? (
        ...
        <div className="rounded-xl border border-dashed border-border bg-card p-12 text-center"><p className="font-medium text-foreground">当前没有活动 Job</p><p className="mt-1 text-sm text-muted-foreground">实时连接断开时才启用低频兜底刷新；可在任务配置中手工启动。</p></div>
      ) : (
            ...
            <article key={job.id} className={`${crawlerPanelClass} p-5`}>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">{job.scheduleName || `配置 #${job.scheduleId}`} · {job.sourceCode || '-'} · {contentTypeLabel(job.contentType)} · {triggerTypeLabel(job.triggerType)}</p>
                </div>
